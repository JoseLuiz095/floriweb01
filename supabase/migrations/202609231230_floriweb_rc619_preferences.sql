-- FloriWeb RC6.19 - preferencias leves de relacionamento e recompra
alter table public.stores
  add column if not exists sales_recovery_window_hours integer not null default 72,
  add column if not exists repeat_order_max_age_days integer not null default 60,
  add column if not exists customer_message_templates jsonb not null default jsonb_build_object(
    'recovery','Olá, {cliente}! Vimos que o pedido {pedido} na {loja} foi registrado, mas o contato não foi concluído. Se ainda quiser finalizar seu presente, responda esta mensagem.',
    'comeback','Olá, {cliente}! A {loja} está por aqui quando quiser preparar um novo presente ou repetir uma escolha especial.'
  );

do $$ begin
  alter table public.stores add constraint stores_sales_recovery_window_hours_check check (sales_recovery_window_hours between 1 and 720);
exception when duplicate_object then null; end $$;
do $$ begin
  alter table public.stores add constraint stores_repeat_order_max_age_days_check check (repeat_order_max_age_days between 1 and 365);
exception when duplicate_object then null; end $$;

-- A vitrine publica precisa conhecer apenas as preferencias que afetam o cliente.
...
begin
  ...
  return jsonb_build_object(
    ...
    'store', jsonb_build_object(
      ...
      'repeat_order_enabled', v_store.repeat_order_enabled,
      'repeat_order_max_age_days', v_store.repeat_order_max_age_days,
