-- FloriWeb v3.0.0-rc.6.16 - visual por emoji em produtos e complementos
-- Migration aditiva. Nao remove colunas nem altera regras de negocio existentes.
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname='products_visual_emoji_length') then
    alter table public.products add constraint products_visual_emoji_length check (visual_emoji is null or char_length(visual_emoji) <= 16);
  end if;
  if not exists (select 1 from pg_constraint where conname='addons_visual_emoji_length') then
    alter table public.addons add constraint addons_visual_emoji_length check (visual_emoji is null or char_length(visual_emoji) <= 16);
  end if;
end $$;
