-- FloriWeb 2.3.0 - formas de pagamento PIX e cartão
-- PIX pode operar por chave tradicional ou por código PIX Copia e Cola base.
-- Cartão é apenas solicitação de link manual via WhatsApp; o FloriWeb não captura dados do cartão.

alter table public.stores
  add column if not exists pix_receipt_mode text not null default 'key',
  add column if not exists pix_copy_paste text,
  add column if not exists card_payment_enabled boolean not null default false;

alter table public.stores
  drop constraint if exists stores_pix_receipt_mode_check;

alter table public.stores
  add constraint stores_pix_receipt_mode_check check (pix_receipt_mode in ('key','copy_paste'));

alter table public.orders
  drop constraint if exists orders_payment_method_check;

alter table public.orders
  add constraint orders_payment_method_check check (payment_method in ('confirm','pix','card'));
...
begin
  ...
  if coalesce(payload->>'payment_method','confirm') not in ('confirm','pix','card') then raise exception 'Forma de pagamento inválida.'; end if;
  if coalesce(payload->>'payment_method','confirm')='pix' and not (v_store.pix_enabled and v_store.show_pix_before_confirmation) then raise exception 'PIX não está disponível nesta loja.'; end if;
  if coalesce(payload->>'payment_method','confirm')='card' and not v_store.card_payment_enabled then raise exception 'Pagamento por cartão não está disponível nesta loja.'; end if;
