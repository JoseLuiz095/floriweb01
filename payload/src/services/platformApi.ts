...
const alignSubscriptionDueDate = (dueDay: number, reference?: string | null) => {
  const configuredDay = Math.max(1, Math.min(31, Math.round(Number(dueDay) || 10)));
  const dateForMonth = (year: number, month: number) => {
    const lastDay = new Date(year, month, 0).getDate();
    const actualDay = Math.min(configuredDay, lastDay);
    return `${String(year).padStart(4, '0')}-${String(month).padStart(2, '0')}-${String(actualDay).padStart(2, '0')}`;
  };
  ...
  const todayKey = [year, String(month).padStart(2, '0'), String(now.getDate()).padStart(2, '0')].join('-');
  if (thisMonth < todayKey) {
    month += 1;
...
const demoPlans: Plan[] = [
  ...
  {id:'pro',code:'PRO',name:'Profissional',productLimit:40,imageLimitPerProduct:6,customDomain:false,reports:true,prioritySupport:true,monthlyPrice:89.9,setupPrice:249,categoryLimit:15,addonLimit:40,adminUserLimit:1,sortOrder:20,active:true},
  {id:'premium',code:'PREMIUM',name:'Premium',productLimit:100,imageLimitPerProduct:10,customDomain:true,reports:true,prioritySupport:true,monthlyPrice:149.9,setupPrice:399,categoryLimit:null,addonLimit:null,adminUserLimit:1,sortOrder:30,active:true},
];
...
export const platformApi = {
  ...
  async updateStoreCredentials(input: { storeId:string; email:string; password?:string; forcePasswordChange?:boolean }): Promise<{ok:boolean;email:string;emailChanged:boolean;passwordChanged:boolean;linkedStoreCount?:number}> {
    ...
    return invokeFunction<{ok:boolean;email:string;emailChanged:boolean;passwordChanged:boolean;linkedStoreCount?:number}>('platform-manage-store-user', input);
  },

  async manageStoreUserFunctionHealth() {
    if (isDemoMode) return {ok:true,function:'platform-manage-store-user',version:'3.0.0-rc.6.9-demo',configured:true};
    return edgeFunctionHealth('platform-manage-store-user');
  },


  async listPlatformEvents(input: { limit?: number; result?: PlatformEventResult; kind?: PlatformEventKind } = {}): Promise<PlatformEventLog[]> {
    if (isDemoMode) return [];
    const rows = await restFetch<PlatformEventLog[]>('rpc/platform_list_event_log_v1', {
      method: 'POST',
      body: {
