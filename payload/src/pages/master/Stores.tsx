...
const isPastDate=(value?:string)=>{if(!value)return false;const now=new Date();const today=[now.getFullYear(),String(now.getMonth()+1).padStart(2,'0'),String(now.getDate()).padStart(2,'0')].join('-');return value.slice(0,10)<today;};
...
const defaultSettings: PlatformSettings={demoEnabled:true,demoDurationDays:30,demoWarningDays:7};
...
export default function MasterStores(){
  ...
  const[credentialFunctionReady,setCredentialFunctionReady]=useState<boolean|null>(null);
  const[credentialFunctionError,setCredentialFunctionError]=useState('');
  const[credentialSaving,setCredentialSaving]=useState(false);
  const[showPassword,setShowPassword]=useState(false);
  const[showEditPassword,setShowEditPassword]=useState(false);
  const[confirmTemporaryPassword,setConfirmTemporaryPassword]=useState('');
  const[editAccessForm,setEditAccessForm]=useState({email:'',password:'',confirmPassword:'',forcePasswordChange:true});
  ...
  const reload=async()=>{
    ...
    try{
      ...
      const selectablePlans=p.filter((plan)=>plan.active&&(settings.demoEnabled||plan.code!=='DEMO'));
      setCreateForm((f)=>({...f,planId:selectablePlans.some((plan)=>plan.id===f.planId)?f.planId:(selectablePlans.find((x)=>x.code==='BASIC')?.id||selectablePlans[0]?.id||'')}));
  ...
  const checkCredentialFunction=async()=>{
    try{
      const health=await platformApi.manageStoreUserFunctionHealth();
      setCredentialFunctionReady(Boolean(health.ok&&health.configured));
      setCredentialFunctionError(health.ok&&health.configured?'':'A função de gestão de acesso está publicada, mas as variáveis internas do Supabase não estão disponíveis.');
    }catch(e){setCredentialFunctionReady(false);setCredentialFunctionError(e instanceof Error?e.message:'Edge Function de gestão de acesso indisponível.')}
  };

  useEffect(()=>{void reload();void checkCreateFunction();void checkCredentialFunction()},[]);
  ...
  const selectableCreatePlans=plans.filter((plan)=>plan.active&&(platformSettings.demoEnabled||plan.code!=='DEMO'));
  ...
  const editEmailError=editAccessForm.email?emailValidationMessage(editAccessForm.email):'';
  const editPasswordError=editAccessForm.password?temporaryPasswordValidationMessage(editAccessForm.password):'';

  const openEdit=(store:PlatformStoreSummary)=>{
    setEditing({...store});
    setEditAccessForm({email:store.ownerEmail||'',password:'',confirmPassword:'',forcePasswordChange:true});
    setShowEditPassword(false);
  };

  const saveCredentials=async()=>{
    if(!editing||credentialSaving)return;
    const validation=emailValidationMessage(editAccessForm.email);
    if(validation){showToast(validation,'error');return;}
    if(editAccessForm.password){
      const passValidation=temporaryPasswordValidationMessage(editAccessForm.password);
      if(passValidation){showToast(passValidation,'error');return;}
      if(editAccessForm.password!==editAccessForm.confirmPassword){showToast('A confirmação da nova senha não confere.','error');return;}
    }
    setCredentialSaving(true);
    try{
      const result=await trackInteraction('store_credentials_update',()=>platformApi.updateStoreCredentials({
        storeId:editing.id,
        email:normalizeEmail(editAccessForm.email),
        password:editAccessForm.password||undefined,
        forcePasswordChange:editAccessForm.forcePasswordChange,
      }),{storeId:editing.id,successKind:'audit'});
      setEditing((current)=>current?{...current,ownerEmail:result.email}:current);
      setEditAccessForm((current)=>({...current,email:result.email,password:'',confirmPassword:''}));
      showToast(result.passwordChanged?'E-mail e/ou senha do lojista atualizados.':'E-mail de acesso atualizado.','success');
      if((result.linkedStoreCount||0)>1)showToast(`Este login está vinculado a ${result.linkedStoreCount} lojas; a alteração de acesso vale para todas elas.`,'info');
      await reload();
    }catch(e){showToast(e instanceof Error?e.message:'Falha ao atualizar e-mail/senha do lojista.','error')}
  ...
  const saveEdit=async()=>{
    ...
    try{
      ...
      await trackInteraction('store_subscription_settings_update',()=>platformApi.updateStore({
  ...
  return <>
    ...
    <section className="admin-card no-padding master-store-table">
      ...
      {loading?<div className="master-table-loading">Carregando...</div>:<div className="responsive-table"><table><thead><tr><th>Loja</th><th>Plano</th><th>Acesso</th><th>Uso</th><th>Mensalidade</th><th>Vencimento</th><th>URL</th><th></th></tr></thead><tbody>{filtered.map((store)=>{
        ...
        const stage=storeCommercialStage(store,platformSettings.demoWarningDays);
        const paymentOverdue=store.planCode!=='DEMO'&&isPastDate(store.nextDueDate);
        return <tr key={store.id} className={isWarning?'trial-warning-row':''}><td><div className="master-store-cell"><span className={`master-store-dot ${store.accessStatus}`}/><div><strong>{store.name}</strong><small>{store.city}/{store.state} · {store.ownerEmail||'sem responsável'}</small><span className={`commercial-stage-badge ${stage.tone}`}>{stage.label}</span>{warning&&<span className={`trial-expiry-badge ${isWarning?'warning':''}`}>{warning}</span>}</div></div></td><td><select className="master-inline-select" aria-label={`Plano de ${store.name}`} value={store.planId||''} disabled={rowSaving===store.id} onChange={(e)=>void quickUpdate(store,{planId:e.target.value})}>{plans.filter((p)=>(p.active||p.id===store.planId)&&(platformSettings.demoEnabled||p.code!=='DEMO'||p.id===store.planId)).map((p)=><option value={p.id} key={p.id}>{p.name}</option>)}</select><small>{store.subscriptionStatus==='trial'?'Teste':store.subscriptionStatus||'—'}</small></td><td><select className={`master-inline-select access-select ${store.accessStatus}`} aria-label={`Status de ${store.name}`} value={store.accessStatus} disabled={rowSaving===store.id} onChange={(e)=>void quickUpdate(store,{accessStatus:e.target.value as StoreAccessStatus})}><option value="online">Online</option><option value="suspended">Desativada</option></select></td><td><strong>{store.activeProductCount} ativos</strong><small>{store.productCount} cadastrados · {store.ownerEmail?'acesso principal configurado':'sem acesso principal'}</small></td><td><strong>{store.planCode==='DEMO'?'Grátis':currency.format(store.billingAmount||0)}</strong><small>{store.planCode==='DEMO'&&store.expiresAt?`até ${new Date(store.expiresAt).toLocaleDateString('pt-BR')}`:'mensal'}</small></td><td><div className={`master-due-cell-rc65 ${paymentOverdue?'is-overdue':''}`}><strong>{store.planCode==='DEMO'?'—':store.dueDay?`Dia ${store.dueDay}`:'Não definido'}</strong><small>{store.planCode==='DEMO'?'Demonstração':store.nextDueDate?`${paymentOverdue?'Atrasado desde':'Próximo'} ${dateBr(store.nextDueDate)}`:'Sem próxima data'}</small></div></td><td><div className="master-url-cell"><a href={publicUrlFor(store)} target="_blank" rel="noreferrer"><ExternalLink size={15}/>{store.customDomain?'Domínio próprio':`/${store.slug}`}</a><button onClick={()=>void copyText(publicUrlFor(store)).then(()=>showToast('Link copiado.','success'))} aria-label="Copiar URL"><Copy size={14}/></button></div></td><td><button className="secondary-button compact-button" onClick={()=>openEdit(store)}>Gerenciar</button></td></tr>})}</tbody></table></div>}
    </section>
    ...
    {creating&&<div className="modal-overlay"><form className="master-modal master-modal--wide" onSubmit={createStore}><button type="button" className="modal-close" onClick={()=>setCreating(false)}><X/></button><span className="eyebrow">NOVA FLORICULTURA</span><h2>Criar loja e acesso do responsável</h2><p>Produtos e pedidos serão isolados pelo novo <code>store_id</code>. Escolha se o cliente receberá convite ou uma senha temporária cadastrada por você.</p>
      ...
      <div className="form-grid">
        ...
        <label>Plano<select value={createForm.planId} onChange={(e)=>{const plan=plans.find((p)=>p.id===e.target.value);setCreateForm((f)=>({...f,planId:e.target.value,customDomain:plan?.customDomain?f.customDomain:''}))}}>{selectableCreatePlans.map((p)=><option value={p.id} key={p.id}>{p.name} · {p.code==='DEMO'?'grátis':`${currency.format(p.monthlyPrice||0)}/mês`}</option>)}</select></label>
        {!platformSettings.demoEnabled&&<div className="full demo-plan-create-note demo-disabled-note"><strong>Novas Demos estão desabilitadas</strong><span>Você pode habilitar novamente em Admin Master → Planos. Clientes que já estão em Demo continuam normalmente.</span></div>}
        ...
        {selectedCreatePlan?.code!=='DEMO'&&<label>Vencimento mensal<input type="number" min="1" max="31" value={createForm.dueDay} onChange={(e)=>setCreateForm((f)=>({...f,dueDay:Number(e.target.value)}))}/></label>}
    ...
    {editing&&<div className="modal-overlay"><div className="master-modal master-modal--wide"><button type="button" className="modal-close" onClick={()=>setEditing(null)}><X/></button><span className="eyebrow">GESTÃO DO CLIENTE</span><h2>{editing.name}</h2><p>Suspender acesso não apaga a loja, produtos, pedidos nem vínculos de usuários. Ao reativar, os acessos anteriores voltam a funcionar.</p><div className="form-grid">
      <label>Plano<select value={editing.planId||''} onChange={(e)=>{const plan=plans.find((p)=>p.id===e.target.value);setEditing((s)=>s?{...s,planId:e.target.value,planName:plan?.name,planCode:plan?.code,billingAmount:plan?.monthlyPrice??s.billingAmount,expiresAt:plan?.code==='DEMO'?addDaysDate(platformSettings.demoDurationDays):undefined,customDomain:plan?.customDomain?s.customDomain:''}:s)}}>{plans.filter((p)=>platformSettings.demoEnabled||p.code!=='DEMO'||p.id===editing.planId).map((p)=><option value={p.id} key={p.id}>{p.name}</option>)}</select></label>
      ...
      {editing.planCode==='DEMO'?<label className="full">Fim da demonstração<input type="date" value={dateOnly(editing.expiresAt)} onChange={(e)=>setEditing((s)=>s?{...s,expiresAt:e.target.value}:s)}/><small>Prazo global atual: {platformSettings.demoDurationDays} dias. Você pode estender ou reduzir este cliente individualmente.</small></label>:<><label>Mensalidade<input type="number" min="0" step="0.01" value={editing.billingAmount||0} onChange={(e)=>setEditing((s)=>s?{...s,billingAmount:Number(e.target.value)}:s)}/></label><label>Dia de vencimento<input type="number" min="1" max="31" value={editing.dueDay||10} onChange={(e)=>setEditing((s)=>s?{...s,dueDay:Number(e.target.value)}:s)}/><small>O próximo vencimento será realinhado para este dia sem seguir a data de um pagamento atrasado.</small></label></>}
    ...
    <div className="credential-setup-card master-credential-edit-rc65"><div className="master-credential-edit-head-rc65"><KeyRound size={20}/><div><span className="eyebrow">ACESSO DO LOJISTA</span><strong>Alterar e-mail ou redefinir senha</strong><small>Esta ação atualiza o usuário do Supabase Auth vinculado à loja. A nova senha é opcional. Se o mesmo login administrar outras lojas, a alteração de e-mail/senha também valerá para elas.</small></div></div>
      {credentialFunctionReady===false&&<div className="master-warning"><TriangleAlert size={18}/><span>{credentialFunctionError||'Publique platform-manage-store-user antes de alterar o acesso.'}</span></div>}
      <div className="temporary-password-grid"><label className="full">E-mail de acesso<input type="email" value={editAccessForm.email} onChange={(e)=>setEditAccessForm((f)=>({...f,email:e.target.value}))}/><small className={editEmailError?'field-help-error':''}>{editEmailError||'Ao salvar, o endereço de login e o e-mail do responsável da loja serão atualizados.'}</small></label><label>Nova senha <span className="optional-label">opcional</span><div className="input-with-icon"><KeyRound size={17}/><input minLength={10} value={editAccessForm.password} onChange={(e)=>setEditAccessForm((f)=>({...f,password:e.target.value}))} type={showEditPassword?'text':'password'} autoComplete="new-password" placeholder="Deixe vazio para manter a atual"/><button type="button" onClick={()=>setShowEditPassword((v)=>!v)} aria-label="Mostrar nova senha">{showEditPassword?<EyeOff size={17}/>:<Eye size={17}/>}</button></div><small className={editPasswordError?'field-help-error':''}>{editPasswordError||'Mínimo 10 caracteres, com maiúscula, minúscula e número.'}</small></label><label>Confirmar nova senha<input minLength={10} value={editAccessForm.confirmPassword} onChange={(e)=>setEditAccessForm((f)=>({...f,confirmPassword:e.target.value}))} type={showEditPassword?'text':'password'} autoComplete="new-password" disabled={!editAccessForm.password}/></label><label className="full checkbox-row"><input type="checkbox" checked={editAccessForm.forcePasswordChange} onChange={(e)=>setEditAccessForm((f)=>({...f,forcePasswordChange:e.target.checked}))} disabled={!editAccessForm.password}/>Exigir que o lojista troque a senha no próximo acesso</label></div>
      <div className="master-credential-edit-action-rc65"><button type="button" className="secondary-button" disabled={credentialSaving||credentialFunctionReady===false||Boolean(editEmailError)||Boolean(editPasswordError)||Boolean(editAccessForm.password&&editAccessForm.password!==editAccessForm.confirmPassword)} onClick={()=>void saveCredentials()}>{credentialSaving?'Atualizando acesso...':'Atualizar e-mail / senha'}</button></div>
    </div>
    <div className="master-modal-actions"><button className="secondary-button" onClick={()=>setEditing(null)}>Cancelar</button><button className="primary-button" disabled={saving} onClick={()=>void saveEdit()}>{saving?'Salvando...':'Salvar alterações'}</button></div></div></div>}
  </>;
}
