...
const alignSubscriptionDueDate = (dueDay: number, reference?: string | null) => {
  const day = Math.max(1, Math.min(28, Math.round(Number(dueDay) || 10)));
  ...
  const next = thisMonth.getTime() >= new Date(year, month, now.getDate()).getTime()
    ? thisMonth
...
export const platformApi = {
  ...
  async manageStoreUserFunctionHealth() {
    if (isDemoMode) return {ok:true,function:'platform-manage-store-user',version:'3.0.0-rc.6.5-demo',configured:true};
