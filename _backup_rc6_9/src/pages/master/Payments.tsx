...
export default function Payments() {
  ...
  const confirm = async (payment: SubscriptionPayment) => {
    ...
    setError('');
    try {
      await confirmSubscriptionPayment(payment.id);
      await load();
    } catch (currentError) {
  ...
  const reject = async () => {
    ...
    setError('');
    try {
      await rejectSubscriptionPayment(rejecting.id, rejectReason.trim());
      setRejecting(null);
      setRejectReason('');
      await load();
    } catch (currentError) {
            ...
            {(payment.status === 'pending' || payment.status === 'proof_sent') && (
              <div className="master-payment-actions-rc65">
                {payment.status === 'proof_sent' && (
                  <button className="primary-button" disabled={busy === payment.id} onClick={() => void confirm(payment)}>
                    <CheckCircle2 size={16} />{busy === payment.id ? 'Confirmando...' : 'Confirmar pagamento'}
                  </button>
                )}
                <button className="secondary-button danger-button-rc65" disabled={busy === payment.id} onClick={() => openReject(payment)}>
