...
ok('Versao V3 RC6.18', packageJson.version === '3.0.0-rc.6.18');
...
const localFinancialReader = read('src/utils/localFinancialDocumentReader.ts');
const financePage = read('src/pages/admin/Finance.tsx');
const ordersPage = read('src/pages/admin/Orders.tsx');
const storeContext = read('src/contexts/StoreContext.tsx');
const orderPaymentMigration = read('supabase/migrations/202609041600_floriweb_rc6_8_order_payment_finance.sql');
const styles = read('src/styles.css');
const masterPayments = read('src/pages/master/Payments.tsx');
const manageStoreUserEdge = read('supabase/functions/platform-manage-store-user/index.ts');
const mRc65 = read('supabase/migrations/202609032030_floriweb_rc6_5_billing_access.sql');
const billingStatus = read('src/utils/billingStatus.ts');
ok('RC6.6 alerta mensalidade verde amarelo vermelho', adminLayout.includes('admin-billing-status-rc66') && adminLayout.includes('is-warning') && billingStatus.includes('Mensalidade atrasada') && billingStatus.includes('Mensalidade em dia') && billingStatus.includes('Vencimento próximo'));
ok('RC6.5 lojista ve ultimo pagamento', adminBilling.includes('ÚLTIMO PAGAMENTO') && adminBilling.includes('Vencimento da referência'));
ok('RC6.6 Meu plano concentra mensalidade e PIX', adminPlan.includes('Meu plano e mensalidade') && adminPlan.includes('PAGAMENTO PENDENTE') && adminPlan.includes('ULTIMO PAGAMENTO') && adminPlan.includes('id="vencimento"'));
ok('RC6.6 topo direciona para Meu plano no vencimento', adminLayout.includes('/admin/plano#vencimento'));
ok('RC6.6 OCR reforca imagem pequena e busca valor por contexto', localFinancialReader.includes('MAX_UPSCALE') && localFinancialReader.includes('LEITURA REFORCADA') && localFinancialReader.includes('VALOR\\s+DO\\s+DOCUMENTO') && localFinancialReader.includes('scoreAmountContext'));
ok('RC6.7 Financeiro esconde inputs nativos e mantem apenas Tirar foto/Escolher arquivo', financePage.includes('hidden type="file"') && financePage.includes('flori-document-actions-rc67') && financePage.includes('Tirar foto') && financePage.includes('Escolher arquivo'));
ok('RC6.7 Financeiro possui leitura visual compacta', styles.includes('flori-document-reader-rc67') && styles.includes('flori-local-read-progress-rc67') && styles.includes('input[hidden]'));
ok('RC6.5 Master pode negar renovacao', masterPayments.includes('Não confirmar renovação') && billingApi.includes('platform_reject_subscription_payment_v1'));
ok('RC6.5 tabela Master mostra vencimento', stores.includes('<th>Vencimento</th>') && stores.includes('master-due-cell-rc65'));
ok('RC6.5 Master altera credenciais', stores.includes('Atualizar e-mail / senha') && platformApi.includes('platform-manage-store-user'));
ok('RC6.5 Edge de credenciais exige MFA', manageStoreUserEdge.includes('MFA_AAL2_REQUIRED') && manageStoreUserEdge.includes('auth.admin.updateUserById'));
ok('RC6.5 vencimento ancora no due_day', mRc65.includes('subscription_reference_due_v1') && mRc65.includes("status='rejected'") && mRc65.includes('next_due_date=v_next_due'));
...
ok('Pedidos possuem atualizacao automatica', read('src/pages/admin/Orders.tsx').includes('setInterval') && read('src/pages/admin/Orders.tsx').includes('visibilitychange') && /reloadAdmin\(\{\s*silent\s*:\s*true\s*\}\)/.test(read('src/pages/admin/Orders.tsx')));
...
ok('RC6.8 pedido possui estado de recebimento', read('src/types/index.ts').includes("OrderPaymentStatus = 'pending' | 'paid'") && storeApi.includes('payment_status') && storeApi.includes('paymentReceivedAt'));
ok('RC6.8 Orders possui botao Confirmar recebimento', ordersPage.includes('Confirmar recebimento') && ordersPage.includes('confirmOrderPayment'));
ok('RC6.8 StoreContext publica confirmacao de recebimento', storeContext.includes('confirmOrderPayment: typeof storeApi.confirmOrderPayment') && storeContext.includes('storeApi.confirmOrderPayment(orderId)'));
ok('RC6.8 confirmacao usa RPC transacional', storeApi.includes("rpc/confirm_order_payment_v1"));
ok('RC6.8 migration cria vinculo pedido-financeiro idempotente', orderPaymentMigration.includes('financial_entries_order_uidx') && orderPaymentMigration.includes('orders_financial_receipt_sync_trg') && orderPaymentMigration.includes('confirm_order_payment_v1'));
ok('RC6.8 receita nasce do recebimento confirmado', orderPaymentMigration.includes("new.payment_status='paid'") && orderPaymentMigration.includes("'income','order'") && orderPaymentMigration.includes("'Vendas'"));
ok('RC6.8 cancelamento invalida receita automatica', orderPaymentMigration.includes("new.status='cancelled'") && orderPaymentMigration.includes("set status='cancelled'"));
ok('RC6.8 possui SQL de validacao', exists('supabase/VALIDAR_RC6_8.sql'));

ok('RC6.9 possui botao explicito Confirmar renovacao', read('src/pages/master/Payments.tsx').includes('Confirmar renovação') && read('src/pages/master/Payments.tsx').includes('Confirmar alteração de plano'));
ok('RC6.9 registra interacoes e erros sem bloquear operacao', exists('src/services/interactionTelemetry.ts') && read('src/main.tsx').includes('installGlobalInteractionTelemetry'));
ok('RC6.9 possui centro de pendencias no Master', read('src/pages/master/Dashboard.tsx').includes('CENTRO DE PENDÊNCIAS') && read('src/pages/master/Dashboard.tsx').includes('listPlatformEvents'));
ok('RC6.9 diagnostico mostra falhas e interacoes sem conclusao', read('src/pages/master/Diagnostics.tsx').includes('Interações sem conclusão') && read('src/pages/master/Diagnostics.tsx').includes('Falhas recentes'));
ok('RC6.9 vencimento aceita dias 1 a 31', read('src/pages/master/Stores.tsx').includes('max="31"') && read('supabase/migrations/202609151030_floriweb_rc6_9_stability_billing_audit.sql').includes('between 1 and 31'));
ok('RC6.9 fevereiro usa ultimo dia sem perder due_day', read('supabase/migrations/202609151030_floriweb_rc6_9_stability_billing_audit.sql').includes('least(v_day,v_last_day)'));
ok('RC6.9 OCR reaproveita worker na sessao', read('src/utils/localFinancialDocumentReader.ts').includes('cachedOcrWorkerPromise') && read('src/utils/localFinancialDocumentReader.ts').includes('pagehide'));
ok('RC6.9 Financeiro destaca origem e campos do OCR', read('src/pages/admin/Finance.tsx').includes('finance-origin-badge-r69') && read('src/pages/admin/Finance.tsx').includes('is-autofilled-r69'));
ok('RC6.9 auditoria critica existe no banco', read('supabase/migrations/202609151030_floriweb_rc6_9_stability_billing_audit.sql').includes('platform_event_log') && read('supabase/migrations/202609151030_floriweb_rc6_9_stability_billing_audit.sql').includes('subscription_renewal_confirmed'));
ok('RC6.9 possui SQL de validacao', exists('supabase/VALIDAR_RC6_9.sql'));
...
const landingPlansRc610 = read('src/pages/store/Landing.tsx');
const masterPlansRc610 = read('src/pages/master/Plans.tsx');
const planMigrationRc610 = read('supabase/migrations/202609151330_floriweb_rc6_10_plan_simplification.sql');
ok('RC6.10 remove promessa de usuarios administrativos', !adminPlan.includes('usuario administrativo') && !adminPlan.includes('usuarios administrativos') && !masterPlansRc610.includes('Usuários admin'));
ok('RC6.10 Business e sob medida e sem preco fixo', adminPlan.includes('Sob consulta') && adminPlan.includes('Dominio proprio incluido') && !adminPlan.includes('R$ 349,90'));
ok('RC6.10 Business aparece na landing', landingPlansRc610.includes('flori-business-plan-rc610') && landingPlansRc610.includes('Quero avaliar um projeto Business'));
...
const storeHeaderRc611 = read('src/components/StoreHeader.tsx');
const masterLayoutRc611 = read('src/layouts/MasterLayout.tsx');
ok('RC6.11 pedidos possuem filtros e ordenacao', ordersPage.includes('orders-toolbar--filters') && ordersPage.includes('Ordenar por') && ordersPage.includes('statusFilter'));
ok('RC6.11 pedido relacionado abre destacado', financePage.includes('/admin/pedidos?highlight=') && ordersPage.includes('order-row-highlighted') && ordersPage.includes('highlightedOrderId'));
ok('RC6.11 horario de hoje com outros dias', storeHeaderRc611.includes('<b>Hoje</b>') && storeHeaderRc611.includes('Outros dias') && storeHeaderRc611.includes('store-hours-popover'));
ok('RC6.11 popover fecha ao clicar fora', storeHeaderRc611.includes('pointerdown') && storeHeaderRc611.includes('hoursRef'));
