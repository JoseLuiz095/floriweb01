...
export default function Finance() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError('');
    try {
      await trackInteraction('financial_entry_create', () => createFinancialEntry(settings.id, {
        direction,
    ...
    <>
              ...
              {data.entries.slice(0, 8).map((entry) => (
                <div className="flori-finance-entry" key={entry.id}>
                  <span className={entry.direction}>
                    ...
                    <span className={`finance-origin-badge-r69 ${entry.source === 'order' ? 'is-order' : 'is-manual'}`}>{entry.source === 'order' ? 'Pedido automático' : entry.source === 'adjustment' ? 'Ajuste' : 'Lançamento manual'}</span>
                    {entry.source === 'order' && entry.orderId && <a className="finance-order-link-r69" href={`/admin/pedidos?highlight=${encodeURIComponent(entry.orderId)}`}>Ver pedido relacionado</a>}
                  </span>
      ...
      <section className="admin-card flori-finance-form">
        ...
        <form onSubmit={submit} className="flori-finance-fields">
          <label className={readResult?.description ? 'is-autofilled-r69' : undefined}>Descrição<span className="autofill-mark-r69">{readResult?.description ? '✓ identificado' : ''}</span><input required value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label>
          <label className={readResult?.amount !== undefined ? 'is-autofilled-r69' : undefined}>Valor<span className="autofill-mark-r69">{readResult?.amount !== undefined ? '✓ identificado' : ''}</span><input required inputMode="decimal" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></label>
          ...
          <label className={readResult?.occurredOn ? 'is-autofilled-r69' : undefined}>Data<span className="autofill-mark-r69">{readResult?.occurredOn ? '✓ identificado' : ''}</span><input type="date" required value={form.occurredOn} onChange={(event) => setForm({ ...form, occurredOn: event.target.value })} /></label>
          ...
          <label className={readResult?.counterparty ? 'is-autofilled-r69' : undefined}>Fornecedor / origem<span className="autofill-mark-r69">{readResult?.counterparty ? '✓ identificado' : ''}</span><input value={form.counterparty} onChange={(event) => setForm({ ...form, counterparty: event.target.value })} /></label>
