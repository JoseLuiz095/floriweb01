...
export default function AdminPlan() {
  ...
  const overdue = subscription?.billingState === 'overdue' || (daysToDue !== undefined && daysToDue < 0);
  const dueSoon = !overdue && daysToDue !== undefined && daysToDue <= BILLING_WARNING_DAYS;
  const healthClass = overdue ? 'is-overdue' : dueSoon ? 'is-warning' : 'is-current';
  const healthTitle = overdue ? 'Mensalidade atrasada' : dueSoon ? 'Vencimento proximo' : 'Mensalidade em dia';
  ...
  return <>
    ...
    {data && subscription?.billingState !== 'trial' && subscription?.billingState !== 'none' && (
      <section id="vencimento" className={`admin-card flori-billing-health-rc66 ${healthClass}`}>
        ...
        <strong>{overdue && subscription.daysOverdue > 0 ? `${subscription.daysOverdue} dia(s) em atraso` : dueSoon && daysToDue !== undefined ? (daysToDue === 0 ? 'Vence hoje' : `${daysToDue} dia(s)`) : `Dia ${subscription.dueDay || '---'}`}</strong>
      </section>
    )}
