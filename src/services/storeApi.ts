...
type ProductRow = { id: string; store_id: string; category_id: string; name: string; slug: string; description: string; price: number | string; promotional_price: number | string | null; active: boolean; featured: boolean; made_to_order: boolean; production_days: number | null; stock_status: Product['stockStatus']; visual_emoji?: string | null; track_stock?: boolean | null; stock_quantity?: number | null; stock_minimum?: number | null; estimated_cost?: number | string | null; technical_sheet?: string | null };
...
type AddonRow = { id: string; store_id: string; name: string; description: string | null; price: number | string; active: boolean; image_url: string | null; image_storage_path: string | null; visual_emoji?: string | null };
...
const mapStore = (r: StoreRow): StoreSettings => ({
  ...
  minimumOrder: toNumber(r.minimum_order), salesRecoveryEnabled:r.sales_recovery_enabled??true, salesRecoveryMinutes:Math.max(5,r.sales_recovery_minutes??30), salesRecoveryWindowHours:Math.min(168,Math.max(1,r.sales_recovery_window_hours??72)), crmEnabled:r.crm_enabled??true, crmComeBackDays:Math.min(365,Math.max(1,r.crm_come_back_days??30)), repeatOrderEnabled:r.repeat_order_enabled??true, repeatOrderMaxAgeDays:Math.min(365,Math.max(1,r.repeat_order_max_age_days??120)), upsellEnabled:r.upsell_enabled??true, upsellLimit:Math.min(6,Math.max(1,r.upsell_limit??3)), messageTemplates:normalizeCustomerMessageTemplates(r.customer_message_templates), openingSchedule: normalizeOpeningSchedule(r.opening_hours), openingHours: (() => { const schedule=normalizeOpeningSchedule(r.opening_hours); const legacy=typeof r.opening_hours === 'string' ? r.opening_hours : ((r.opening_hours as { display?: string } | null)?.display || ''); return schedule.days.some((d)=>d.enabled) ? formatOpeningSchedule(schedule) : legacy; })(), active: r.active, accessStatus: r.access_status || 'online',
});
...
const mapAddon = (r: AddonRow): Addon => ({ id: r.id, storeId: r.store_id, name: r.name, description: r.description || undefined, price: toNumber(r.price), active: r.active, imageUrl: r.image_url || undefined, imageStoragePath: r.image_storage_path || undefined, visualEmoji: r.visual_emoji || undefined });
...
const buildProducts = (rows: ProductRow[], images: ProductImageRow[], variants: ProductVariantRow[], addons: Addon[], links: ProductAddonRow[]): Product[] => rows.map((r) => {
  ...
  return {
    ...
    trackStock:r.track_stock ?? false, stockQuantity:r.stock_quantity ?? undefined, stockMinimum:r.stock_minimum ?? undefined, estimatedCost:r.estimated_cost == null ? undefined : toNumber(r.estimated_cost), technicalSheet:r.technical_sheet || undefined,
    images: productImages, imageUrl: primary?.url || '/assets/placeholder-flower.svg', gallery: productImages.map((image)=>image.url), visualEmoji:r.visual_emoji || undefined,
...
export const storeApi = {
  ...
  async saveAddon(addon: Addon): Promise<Addon> {
    ...
    const payload={id:addon.id,store_id:addon.storeId,name:addon.name,description:addon.description||null,price:addon.price,active:addon.active,image_url:addon.imageUrl||null,image_storage_path:addon.imageStoragePath||null,visual_emoji:addon.visualEmoji||null};
  ...
  async confirmOrderPayment(orderId:string):Promise<{alreadyPaid:boolean;paymentReceivedAt?:string}>{
    if(isDemoMode){
      ...
      if(db.orders[index].status==='cancelled')throw new Error('Não é possível confirmar o recebimento de um pedido cancelado.');
      const alreadyPaid=db.orders[index].paymentStatus==='paid';
      const receivedAt=db.orders[index].paymentReceivedAt||new Date().toISOString();
      db.orders[index]={...db.orders[index],paymentStatus:'paid',paymentReceivedAt:receivedAt};
      writeDemo(db);
      return {alreadyPaid,paymentReceivedAt:receivedAt};
    }
    const result=await restFetch<{ok:boolean;alreadyPaid?:boolean;paymentReceivedAt?:string}>('rpc/confirm_order_payment_v1',{method:'POST',body:{p_order_id:orderId}});
    return {alreadyPaid:Boolean(result?.alreadyPaid),paymentReceivedAt:result?.paymentReceivedAt};
  },
