...
export default function ProductDetail(){
  ...
  return <div className="simple-page"><header className="simple-topbar container"><a href={storefrontPath(storeBasePath)}><ArrowLeft size={19}/>Voltar ao catálogo</a><button onClick={()=>void share()}><Share2 size={18}/>Compartilhar</button></header><div className="container product-detail-layout">
    <section className="product-gallery"><div className="product-main-image"><ProductMedia imageUrl={gallery[imageIndex]||gallery[0]} visualEmoji={product.visualEmoji} alt={product.name} wrapperClassName="product-detail-media"/>{product.madeToOrder&&<span className="gallery-badge"><Clock3 size={14}/>Sob encomenda</span>}</div>{gallery.length>1&&<div className="gallery-thumbs">{gallery.map((src,index)=><button key={`${src}-${index}`} className={imageIndex===index?'active':''} onClick={()=>setImageIndex(index)}><ImageWithFallback src={src} alt={`${product.name} ${index+1}`}/></button>)}</div>}</section>
